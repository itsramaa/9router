...
class ServerHandlers:
    ...
    async def handle_accounts(self, request: web.Request) -> web.Response:
        ...
        from core.safe_types import strip_passwords  # AUDIT-002: Use SafeAccount type
        ...
        # AUDIT-002: Strip passwords using SafeAccount type
        safe = strip_passwords(accounts)
    ...
    async def handle_start(self, request: web.Request) -> web.Response:
        ...
        await self.state.cleanup_retry()  # AUDIT-001: Now async with lock
    ...
    async def handle_stop(self, request: web.Request) -> web.Response:
        ...
        # AUDIT-001 fix: Terminate retry processes under lock
        async with self.state.retry_lock:
    ...
    async def _stream_retry_proc(
        ...
        try:
            ...
            # AUDIT-001 fix: Cleanup under lock
            async with self.state.retry_lock:
    ...
    async def handle_bulk_harvest(self, request: web.Request) -> web.Response:
        ...
        # AUDIT-005 fix: Use random temp filename to prevent race conditions
        import secrets

        temp_accounts = self.outputs / f"bulk-harvest-{secrets.token_hex(8)}.json"
        ...
        # AUDIT-005 fix: Cleanup temp file AFTER process completes, not immediately
        async def _cleanup_temp_file():
            """Wait for process to complete, then cleanup temp accounts file."""
            try:
                await self.state.proc.wait()
            except Exception:
                pass
            finally:
                try:
        ...
        asyncio.create_task(_cleanup_temp_file())
