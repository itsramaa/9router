...
async function canAccessLocalOnlyRoute(request) {
  ...
  // Local browser (non-Docker): loopback peer IP + auth

  if (isLocalRequest(request) && (await isAuthenticated(request))) return true;

  // Docker: browser from host machine has non-loopback TCP socket IP (x-9r-real-ip)

  // but NO x-9r-via-proxy header (direct connection, not reverse proxy). The browser

  // URL bar still says localhost:20128, so Host header is loopback. Tunnels always

  // set x-9r-via-proxy=1 via custom-server.js, blocking tunnel access here.
