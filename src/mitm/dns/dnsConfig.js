...
// When running inside Docker on Windows, the container's /etc/hosts is not the

// Windows host's hosts file. The docker-compose can mount the Windows hosts file

// at /etc/hosts-windows (read-only). If that file exists, use it for DNS detection

// so the UI reflects what the Windows host actually has configured.

const DOCKER_WIN_HOSTS = '/etc/hosts-windows';

function isDockerOnWindows() {
  if (IS_WIN) return false;

  return fs.existsSync(DOCKER_WIN_HOSTS);
}

function getEffectiveHostsFile() {
  return isDockerOnWindows() ? DOCKER_WIN_HOSTS : HOSTS_FILE;
}
 ...
 * In Docker-on-Windows mode, reads the mounted Windows hosts file at

 * /etc/hosts-windows instead of the container's /etc/hosts.

 */

function checkDNSEntry(host = null) {
  try {
    const hostsContent = fs.readFileSync(getEffectiveHostsFile(), 'utf8');
 ...
 * In Docker-on-Windows mode, reads the mounted Windows hosts file.
 ...
 * Returns true when DNS must be configured manually on the Windows host

 * (i.e. we're running inside Docker on Windows).

 * Exported so the API layer can include this flag in status responses.

 */

function isManualDnsMode() {
