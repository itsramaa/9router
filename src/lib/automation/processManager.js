...
export function resolveScriptsDir(override) {
  ...
  return (
    override?.trim() || process.env.AUTOMATION_SCRIPTS_DIR?.trim() || bundled
  );
}
...
function spawnSetupCmd(python, args, cwd, key) {
  return new Promise((resolve, reject) => {
    ...
    proc.on('error', (err) => {
      ...
      pushLog(`[setup] Error: ${err.message}`, 'system');
    ...
    proc.on('exit', (code) => {
      ...
      pushLog(
        `[setup] ${key} — ${code === 0 ? '✓ done' : '✗ failed (code ' + code + ')'}`,
...
async function checkSetupInstalled(python, cwd) {
  ...
  // Check camoufox: just try to import the package — if importable, it's installed
  const camoufoxOk = await check(['-c', 'import camoufox']);
  _setup.camoufox = camoufoxOk ? 'done' : 'idle';
  _state.emitter.emit('setup', { ..._setup });
  pushLog(
    `[setup] Check complete — deps:${_setup.deps} camoufox:${_setup.camoufox}`,
...
export async function runSetup(action, { pythonPath, scriptsDir } = {}) {
  ...
  if (action === 'create-venv') {
    const sysPython =
      pythonPath?.trim() || (os.platform() === 'win32' ? 'python' : 'python3');
    ...
    return new Promise((resolve, reject) => {
      ...
      proc.on('error', (e) => {
        pushLog(`[setup] venv error: ${e.message}`, 'system');
  ...
  if (action === 'install-deps') {
    ...
    return spawnSetupCmd(
      python,

      ['-m', 'pip', 'install', '-r', 'requirements-harvest.txt'],
  ...
  if (action === 'install-camoufox') {
    ...
    pushLog(
      '[setup] Downloading Camoufox browser (this may take a few minutes)...',
...
export async function startServer({ pythonPath, scriptsDir, port } = {}) {
  ...
  // Kill any stale Python server left from a previous Node.js session (e.g. hot-reload)

  const pidFile = path.join(resolvedDir, 'server.pid');
  ...
  if (fs.existsSync(pidFile)) {
    ...
    if (!isNaN(oldPid)) {
      try {
        ...
        pushLog(
          `[manager] Killing stale server process (pid ${oldPid})...`,
          'system'
        );
        if (os.platform() === 'win32') {
          // process.kill() only works within the same Windows session;
          // taskkill /F works cross-session (e.g. after hot-reload).
          spawn('taskkill', ['/F', '/PID', String(oldPid)], {
            windowsHide: true,
            stdio: 'ignore',
          });
  ...
  try {
    const { getAutomationAccountsForSync } =
      await import('../db/repos/automationAccountsRepo.js');

    const accounts = await getAutomationAccountsForSync();

    // Always write — even empty — so a stale accounts.json from a previous
    // import is never used after a clear-all + restart cycle.
    ...
    pushLog(
      `[manager] Synced ${accounts.length} account(s) → accounts.json`,
  ...
  const proc = spawn(
    ...
    ['server.py', '--port', String(resolvedPort)],

    {
      ...
      env: {
        ...process.env,
        // Pass the actual Next.js dashboard URL so harvester injects keys to the right port
        DASHBOARD_BASE_URL:
  ...
  // Persist PID so next startServer() can kill this orphan if Node.js restarts
  try {
  ...
  proc.on('exit', (code, signal) => {
    ...
    // On Windows, killed processes exit with code 1 and signal=null.

    // Treat any exit that happened while we were stopping as clean.

    const wasStoppingOrClean =
      _state.status === 'stopping' || code === 0 || signal === 'SIGTERM';
    ...
    setStatus(wasStoppingOrClean ? 'stopped' : 'error');
  });
...
export function stopServer() {
  ...
  const port = _state.port;

  // Call /api/stop first so Python cleans up the harvest subprocess (prevents orphans on Windows).
  // Fire-and-forget with 2s timeout, then kill the server regardless.
  (async () => {
    try {
