...
// ── Setup state ──────────────────────────────────────────────────────────────
const _setup = {
  deps:     "idle",     // "idle" | "running" | "done" | "error"
  camoufox: "idle",
};
...
async function checkSetupInstalled(python, cwd) {
  // Use pip show — more reliable than import (works even if import has side effects)
  ...
  // camoufox browser: check if the fetch was done (Firefox binary exists)
  const camoufoxOk = await check(["-c",
    "import camoufox, os, sys; p=camoufox.get_target_path(); sys.exit(0 if p and os.path.exists(p) else 1)"
  ]);
...
export async function startServer({ pythonPath, scriptsDir, port } = {}) {
  ...
  // Sync accounts DB → accounts.json before spawn
  try {
