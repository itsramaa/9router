...
async function runHeavyStartup() {
  ...
  // Start daily account check scheduler
  import("@/shared/services/dailyAccountCheck")
    .then(({ startDailyAccountCheck }) => {
      startDailyAccountCheck();
      console.log("[InitApp] Daily account check scheduler started");
    })
    .catch((e) => console.log("[InitApp] Daily account check start failed:", e.message));

  // Start model lock cleanup scheduler (hourly)
  import("open-sse/services/modelLockCleanup")
    .then(({ runModelLockCleanup }) => {
      // Register hourly cleanup task
      import("@/shared/services/usageScheduler")
        .then(({ register, start }) => {
          ...
          start();
          console.log("[InitApp] Model lock cleanup scheduler registered");
        })
        .catch((e) => console.log("[InitApp] Model lock cleanup registration failed:", e.message));
    })
    .catch((e) => console.log("[InitApp] Model lock cleanup import failed:", e.message));

  // Start quota monitor scheduler (every 10 minutes)
  import("open-sse/services/quotaMonitor")
    .then(({ runQuotaMonitorTick, QUOTA_SUPPORTED_PROVIDERS }) => {
      import("@/shared/services/usageScheduler")
        .then(({ register, start }) => {
          ...
          start();
          console.log("[InitApp] Quota monitor scheduler registered for providers:", [...QUOTA_SUPPORTED_PROVIDERS].join(', '));
        })
        .catch((e) => console.log("[InitApp] Quota monitor registration failed:", e.message));
    })
    .catch((e) => console.log("[InitApp] Quota monitor import failed:", e.message));
}
