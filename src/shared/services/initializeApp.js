...
function startLifecycleJobs() {
  const gLife = (global.__lifecycleJobs ??= {
    pauseRecovery: null,
    modelLockCleanup: null,
    dailyStarted: false,
  });

  if (!gLife.dailyStarted) {
    gLife.dailyStarted = true;
    import("@/shared/services/dailyAccountCheck")
      .then(({ startDailyAccountCheck }) => startDailyAccountCheck())
      .catch((e) => console.log("[DailyCheck] start failed:", e.message));
  }

  if (!gLife.pauseRecovery) {
    const tickPause = async () => {
      try {
        const { resumeExpiredPauses } = await import("@/shared/services/accountLifecycle.js");
        const inactive = await getProviderConnections({ isActive: false });
        const now = Date.now();
        const providers = new Set(
          inactive
            .filter((c) => c.pausedUntil && new Date(c.pausedUntil).getTime() <= now)
            .map((c) => c.provider)
            .filter(Boolean),
        );
        ...
        console.warn(`[PauseRecovery] tick failed: ${e.message}`);
      }
    };
    gLife.pauseRecovery = setInterval(tickPause, 5 * 60 * 1000);
    if (gLife.pauseRecovery.unref) gLife.pauseRecovery.unref();
    tickPause().catch(() => {});
  }

  if (!gLife.modelLockCleanup) {
    const tickLocks = () =>
      import("open-sse/services/modelLockCleanup.js")
        .then((m) => m.runModelLockCleanup())
        .catch((e) => console.warn(`[ModelLockCleanup] ${e.message}`));
    gLife.modelLockCleanup = setInterval(tickLocks, 60 * 60 * 1000);
    if (gLife.modelLockCleanup.unref) gLife.modelLockCleanup.unref();
  }
}
