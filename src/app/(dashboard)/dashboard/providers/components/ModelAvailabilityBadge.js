...
export default function ModelAvailabilityBadge() {
  ...
  useEffect(() => {
    // BUG-12 fix: the trigger button is commented out so the popover can never
    // be opened — polling is wasteful and produces no visible benefit.
    // Fetch once on mount so data is ready if the button is re-enabled later,
    // but do NOT start a 30s interval.
    fetchStatus();
    // NOTE: when the button is uncommented, restore the interval here:
