...
export default function ProviderDetailPage() {
  ...
  const handleUpdateConnectionStatus = async (id, isActive) => {
    try {
      ...
      if (res.ok) {
        // BUG-06/20 fix: After the server executes activate()/deactivate(), it will clear fields such as modelLock_* and testStatus.
        // The client needs to re-fetch to obtain the complete latest state, instead of just optimistically updating isActive.
        await fetchConnections();
      }
    } catch (error) {
