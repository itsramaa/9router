...
export default function ProviderDetailPage() {
  ...
  const handleBulkTest = async () => {
    ...
    try {
      for (let index = 0; index < selectedConns.length; index += 1) {
        if (stopOneByOneRef.current) {
          setOneByOneSummary({
            total: selectedConns.length,
            completed: index,
            passed,
            failed,
            stopped: true,
        ...
        const connection = selectedConns[index];
        setOneByOneCurrentConnectionId(connection.id);
        setOneByOneResults((prev) => ({
          ...prev,
          [connection.id]: { state: "testing", error: null },
        ...
        if (index < selectedConns.length - 1) {
          await sleep(ONE_BY_ONE_DELAY_MS);
        }
      }
    } finally {
                  ...
                  {selectedConnectionIds.length > 0 && (
                    <>
                      ...
                      <button
                        ...
                        disabled={oneByOneRunning}
                        className="flex items-center gap-1 rounded px-2 py-1 text-xs text-text-muted hover:bg-black/5 hover:text-primary dark:hover:bg-white/5 disabled:opacity-50"
                        title="Test selected connections"
                      >
                      ...
                      <button
                        ...
                        className="flex items-center gap-1 rounded px-2 py-1 text-xs text-text-muted hover:bg-black/5 hover:text-red-500 dark:hover:bg-white/5"
