...
export default function ConnectionRow({
  ...
  const isBanned =
    connection.isActive === false && isBannedError(connection.lastError);
  ...
  const getOneByOneLabel = () => {
    ...
    if (oneByOneStatus.state === "success") {
      ...
      if (diag && diag.type === "quota_warning")
        return "success (quota warning)";
      if (diag && diag.type === "model_locked")
        return `success (locked: ${diag.activeLocks?.join(", ")})`;
      return "success";
    }
      ...
      <div className="flex min-w-0 flex-1 items-start gap-2 sm:items-center sm:gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="mt-1 flex min-w-0 flex-wrap items-center gap-1.5 sm:gap-2">
            ...
            {isPaused && pausedUntil && <CooldownTimer until={pausedUntil} />}
            ...
            {isPaused && connection.lastError && (
              <span
                className="max-w-full truncate text-xs text-orange-500 sm:max-w-[300px]"
                title={connection.lastError}
              >
                {connection.lastError}
              </span>
