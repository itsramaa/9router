...
export default function LiveLog({ entries, onClear }) {
  ...
  function handleCopy() {
    ...
    const text = entries
      .map((e) => `[${e.ts ?? ''}] ${e.provider ? `[${e.provider}] ` : ''}${e.message ?? e.error ?? ''}`)
      .join('\n');
    navigator.clipboard.writeText(text).catch(() => { });
    copyRef.current = true;
    setTimeout(() => { copyRef.current = false; }, 1500);
  }
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-border-subtle shrink-0">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="text-xs text-text-muted hover:text-text-main transition-colors cursor-pointer flex items-center gap-1 disabled:opacity-40"
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar min-h-[200px] max-h-[340px] px-3 py-2 font-mono text-[11px] space-y-0.5 bg-surface-2/30">
        {entries.length === 0 ? (
          <p className="text-text-muted text-center py-8">
            No log entries yet. Start a harvest to see output.
          </p>
        ) : (
          entries.map((e, i) => (
            <div
              ...
              className={cn(
                ...
                TYPE_STYLES[e.type] ?? TYPE_STYLES.log
              )}
            >
              ...
              {e.email && (
                <span className="px-1 rounded bg-surface-2 text-text-muted shrink-0 truncate max-w-[180px]" title={e.email}>
                  {e.email}
                </span>
              )}
              ...
              {e.slot != null && (
                <span className="px-1 rounded bg-surface-2 text-text-muted shrink-0 ml-auto">
                  [slot {e.slot}]
                </span>
              )}
            </div>
