...
function RemoteControl({ slot, frame }) {
  ...
  const send = useCallback(async (action) => {
    setBusy(true);
    try { await sendInteract(slot.index, action); }
  ...
  return (
    <div className="flex flex-col gap-3 p-3 overflow-y-auto">
      ...
      >
        {frame ? (
          ...
          <div className="flex flex-col items-center justify-center py-10 gap-2 text-white/30">
            ...
            <p className="text-[10px]">Click Refresh to capture current state</p>
          </div>
        )}
        {frame && (
      ...
      {/* Navigate */}
      <div>
        <label className="text-[10px] font-semibold text-white/40 uppercase tracking-wider block mb-1">Navigate to URL</label>
        <div className="flex gap-2">
          <input
            type="text" value={gotoUrl}
          ...
          />
          <button
            onClick={handleGoto} disabled={busy || !gotoUrl.trim()}
      ...
      {/* Type text */}
      <div>
        <label className="text-[10px] font-semibold text-white/40 uppercase tracking-wider block mb-1">Type Text</label>
        <div className="flex gap-2">
          <input
            type="text" value={typeText}
          ...
          />
          <button
            onClick={handleType} disabled={busy || !typeText}
            className="px-3 py-1.5 rounded-lg bg-primary/20 text-primary text-xs font-semibold hover:bg-primary/30 transition-colors disabled:opacity-40 cursor-pointer shrink-0"
          >Type</button>
        </div>
      </div>
      ...
      {/* Page actions */}
      <div>
        <label className="text-[10px] font-semibold text-white/40 uppercase tracking-wider block mb-1">Page Actions</label>
        <div className="flex flex-wrap gap-2">
          {[
            { action: 'screenshot', icon: 'screenshot_monitor', label: 'Refresh' },
            { action: 'back',       icon: 'arrow_back',         label: 'Back' },
            { action: 'reload',     icon: 'refresh',            label: 'Reload' },
          ].map((a) => (
            <button
              key={a.action} onClick={() => send(a.action)} disabled={busy}
      ...
      {slot.status === 'interact' && (
        <div className="border-t border-white/10 pt-3 flex gap-2 flex-wrap">
          ...
          {[
            { action: 'continue', label: 'Continue', cls: 'bg-blue-500 text-white hover:bg-blue-600' },
            { action: 'retry',    label: 'Retry',    cls: 'border border-white/20 text-white/70 hover:bg-white/10' },
            { action: 'skip',     label: 'Skip',     cls: 'border border-white/20 text-white/70 hover:bg-white/10' },
            { action: 'abort',    label: 'Abort',    cls: 'border border-red-500/30 text-red-400 hover:bg-red-500/10' },
          ].map((a) => (
            <button
              key={a.action} onClick={() => send(a.action)} disabled={busy}
              className={cn('px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors disabled:opacity-40 cursor-pointer', a.cls)}
            >
              {a.label}
            </button>
          ))}
...
function SlotZoomModal({ slots, frames, logEntries = [], activeIndex, onIndexChange, onClose, onInteract }) {
  ...
  const slotLogs = logEntries.filter((e) => e.slot == null || e.slot === slot?.index);
  ...
  useEffect(() => {
    function onKey(e) {
      ...
      if (e.key === 'ArrowRight' && activeIndex < total - 1) onIndexChange(activeIndex + 1);
    }
  ...
  return (
    <div className="fixed inset-0 z-[70] flex flex-col bg-black/90 backdrop-blur-sm" onClick={onClose}>
      ...
      {/* Header */}
      <div className="flex items-center gap-3 px-5 py-3 border-b border-white/10 shrink-0" onClick={(e) => e.stopPropagation()}>
        <span className={cn('w-2 h-2 rounded-full shrink-0', st.dot)} />
        <span className="text-sm font-semibold text-white">Slot {slot.index}</span>
        <span className={cn('text-xs font-medium px-2 py-0.5 rounded-full border', {
          'border-amber-400/40 text-amber-400 bg-amber-400/10': slot.status === 'running',
          'border-blue-500/40 text-blue-400 bg-blue-500/10':    slot.status === 'interact',
          'border-green-500/30 text-green-400 bg-green-500/10': slot.status === 'done',
          'border-red-500/30 text-red-400 bg-red-500/10':       slot.status === 'error',
          'border-white/20 text-white/50': !['running','interact','done','error'].includes(slot.status),
        })}>
          {st.label}
        </span>
        {slot.email    && <span className="text-xs font-mono text-white/60 truncate max-w-[200px]">{slot.email}</span>}
        {slot.provider && <span className="text-xs text-blue-400/80 truncate">{slot.provider}</span>}
        <div className="flex-1" />
        {total > 1 && <span className="text-xs text-white/50 font-mono shrink-0">{activeIndex + 1} / {total}</span>}
        ...
        <button
          ...
          className={cn(
            ...
            showLog ? 'border-primary/40 bg-primary/10 text-primary' : 'border-white/20 text-white/40 hover:text-white/70'
          )}
        >
          <span className="material-symbols-outlined text-[14px]">terminal</span>
          <span className="hidden sm:inline">Log</span>
          {slotLogs.length > 0 && (
            <span className="px-1 rounded-full bg-white/10 text-[10px] font-mono">{slotLogs.length}</span>
          )}
        </button>

        <button onClick={onClose} className="text-white/60 hover:text-white transition-colors cursor-pointer shrink-0">
            ...
            <div className="flex items-center justify-center p-4 h-full min-h-[200px]">
              {frame ? (
                ...
                <div className="flex flex-col items-center gap-3 text-white/30">
                  <span className="material-symbols-outlined text-[48px]">monitor</span>
                  ...
                  >
                    <span className="material-symbols-outlined text-[13px]">computer</span>
                    Open Remote Control
                  </button>
                </div>
              )}
        ...
        {showLog && (
          <div className="w-72 shrink-0 border-l border-white/10 flex flex-col min-h-0">
            <div className="flex items-center gap-2 px-3 py-2 border-b border-white/10 shrink-0">
              <span className="material-symbols-outlined text-[13px] text-white/40">terminal</span>
              <span className="text-[11px] font-semibold text-white/50 uppercase tracking-wider">
                Slot {slot.index} Log
              </span>
              <span className="ml-auto text-[10px] text-white/30 font-mono">{slotLogs.length}</span>
            </div>
            <div className="flex-1 overflow-y-auto px-2 py-1.5 space-y-0.5 font-mono text-[10px]">
              {slotLogs.length === 0 ? (
                <p className="text-white/20 text-center py-6">No log entries yet.</p>
              ) : (
                slotLogs.map((e, i) => (
                  <div key={i} className={cn('flex items-start gap-1 leading-relaxed', LOG_COLORS[e.type] ?? LOG_COLORS.log)}>
                    ...
                    {e.provider && <span className="text-blue-400/60 shrink-0">[{e.provider}]</span>}
      ...
      {total > 1 && (
        <div className="shrink-0 px-5 py-3 border-t border-white/10 flex items-center gap-4" onClick={(e) => e.stopPropagation()}>
          <button
            onClick={() => activeIndex > 0 && onIndexChange(activeIndex - 1)} disabled={activeIndex === 0}
          ...
          <div className="flex-1 flex flex-col gap-1.5">
            ...
            <div className="flex gap-1.5 justify-center flex-wrap">
              {slots.map((s, i) => {
                ...
                return (
                  <button
                    key={s.index} onClick={() => onIndexChange(i)}
                    className={cn(
                  ...
                  >
                    <span className={cn('w-1.5 h-1.5 rounded-full shrink-0', tst.dot)} />
                    {s.index}
                  </button>
                );
              })}
          ...
          <button
            onClick={() => activeIndex < total - 1 && onIndexChange(activeIndex + 1)} disabled={activeIndex === total - 1}
            className="text-white/60 hover:text-white disabled:opacity-20 transition-colors cursor-pointer shrink-0"
          >
...
export default function SlotDetail({ slot, frame, onInteract, slots, frames, slotIndex, logEntries }) {
  ...
  return (
    <>
      <div
        className={cn('rounded-xl border overflow-hidden transition-colors cursor-zoom-in', st.border, st.bg)}
        onClick={openZoom}
        title="Click to zoom"
      >
        {/* Header */}
        <div className="flex items-center gap-2 px-3 py-2 border-b border-border-subtle/50">
          ...
          <span className="material-symbols-outlined text-[11px] text-text-muted/40 ml-0.5">zoom_in</span>
        ...
        {slot.status === 'interact' && onInteract && (
          <div className="px-3 pb-2" onClick={(e) => e.stopPropagation()}>
            <button
              onClick={(e) => { e.stopPropagation(); onInteract(slot.index); }}
