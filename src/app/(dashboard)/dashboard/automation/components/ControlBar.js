...
const RUN_STATES = {
  idle:     { label: "Idle",     color: "text-text-muted",                   dot: "bg-surface-2 border border-border-subtle" },
  running:  { label: "Running",  color: "text-green-600 dark:text-green-400", dot: "bg-green-500 animate-pulse" },
  stopping: { label: "Stopping", color: "text-amber-600 dark:text-amber-400", dot: "bg-amber-400 animate-pulse" },
  done:     { label: "Done",     color: "text-blue-600 dark:text-blue-400",   dot: "bg-blue-500" },
};

export default function ControlBar({ runState, onStart, onStop, onReset, disabled }) {
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-3 rounded-[14px] border border-border-subtle bg-surface shadow-[var(--shadow-soft)] flex-wrap">
      ...
      <div className="flex items-center gap-2 mr-auto">
        ...
        <span className={cn("text-xs font-semibold", rs.color)}>{rs.label}</span>
      </div>
