...
export default function MitmToolCard({
  ...
  // In manual DNS mode (Docker on Windows), model mappings are always editable
  const mappingsEditable = dnsActive || manualDnsMode;
            ...
            <div className="flex flex-col gap-0.5 text-[11px] text-text-muted px-1">
              ...
              {!mappingsEditable && (
                ...
                <p className="text-amber-600 text-[10px] mt-1">
                  ...
                  ⚠️ Enable DNS to edit model mappings (or add hosts entries manually on Windows)
                  ...
                  <div key={model.alias} className="grid grid-cols-1 gap-1.5 sm:grid-cols-[9rem_auto_1fr_auto] sm:items-center sm:gap-2">
                    ...
                    <div className="relative w-full min-w-0">
                      ...
                      <input
                        ...
                        disabled={!mappingsEditable}

                        className={`w-full min-w-0 pl-2 pr-7 py-2 bg-surface rounded border border-border text-xs focus:outline-none focus:ring-1 focus:ring-primary/50 sm:py-1.5 ${!mappingsEditable ? "opacity-50 cursor-not-allowed" : ""}`}
                    ...
                    <button
                      ...
                      className={`rounded border px-2 py-2 text-xs transition-colors sm:py-1.5 ${hasActiveProviders && mappingsEditable ? "bg-surface border-border hover:border-primary cursor-pointer" : "opacity-50 cursor-not-allowed border-border"}`}
