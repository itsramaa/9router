  ...
  const handleToggleConnectionActive = useCallback(
    async (id, isActive) => {
      ...
      try {
        ...
        if (res.ok) {
          // BUG-13 fix: removed no-op setQuotaData; added fetchQuota for newly
          // enabled connections so the quota panel shows fresh data immediately
          const updatedConns = await reconcileConnectionsPage(fetchConnections, page);
          // If the connection was just enabled and is a supported provider,
          // fetch its quota so the panel shows data without waiting for auto-refresh
          if (isActive) {
            const conn = (updatedConns || []).find((c) => c.id === id);
            if (conn && USAGE_SUPPORTED_PROVIDERS.includes(conn.provider)) {
