...
export async function PUT(request, { params }) {
  try {
    ...
    if (body.disabledByProviderToggle !== undefined) {
      updateData.disabledByProviderToggle = body.disabledByProviderToggle ?? null;
    }
    ...
    if (isActive !== undefined && isActive !== existing.isActive) {
      try {
        const { activate, deactivate } = await import('@/shared/services/accountLifecycle');
        if (isActive) {
          await activate(id);
        } else {
