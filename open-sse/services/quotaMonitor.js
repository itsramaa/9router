...
// Decision thresholds (D1 from OpenSpec)
const LOCK_VS_PAUSE_THRESHOLD_MS = 60 * 60 * 1000;     // 1h: below this → lock, above → pause
...
export function analyzeQuota(provider, usageData) {
  ...
  if (provider === "claude") {
    ...
    if (weekly && typeof weekly.remaining === "number" && weekly.remaining === 0 && weekly.resetAt) {
      return {
        exhausted: true,
    ...
    if (session && typeof session.remaining === "number" && session.remaining === 0 && session.resetAt) {
      return {
        exhausted: true,
  ...
  if (provider === "github") {
    ...
    if (nonUnlimited.length > 0 && nonUnlimited.every(q => (q.remaining ?? 1) === 0)) {
  ...
  if (provider === "codex") {
    if (usageData.limitReached === true) {
      const sessionReset = usageData.quotas?.session?.resetAt || usageData.quotas?.["session"]?.resetAt;
      const weeklyReset = usageData.quotas?.weekly?.resetAt || usageData.quotas?.["weekly"]?.resetAt;
  ...
  if (provider === "kiro") {
    const quotas = usageData.quotas || {};
    const exhausted = Object.values(quotas).find(q => q && !q.unlimited && (q.remaining ?? 1) === 0);
    if (exhausted) {
      return {
        exhausted: true,
  ...
  if (provider === "antigravity" || provider === "gemini-cli") {
    ...
    for (const [modelId, q] of Object.entries(quotas)) {
      ...
      const remainingPct = typeof q.remainingPercentage === "number" ? q.remainingPercentage : (q.remaining ?? 1);
      if (remainingPct === 0) {
...
export function isQuotaRecovered(provider, usageData) {
  ...
  return values.some(q => (typeof q.remaining === "number" ? q.remaining : (q.remainingPercentage ?? 1)) > 0);
}
 ...
 * - Phase 2: Check recovery for manually-paused connections and auto-resume if quota recovered.
 */
export async function runQuotaMonitorTick() {
  ...
  for (const provider of QUOTA_SUPPORTED_PROVIDERS) {
    ...
    try {
      ...
      console.warn(`[QuotaMonitor] failed to fetch connections for ${provider}: ${e.message}`);
      continue;
    }
      ...
      try {
        ...
        if (!status.exhausted) {
          ...
          if (conn.quotaStatus === "exhausted") {
            ...
            console.log(`[QuotaMonitor] ${provider}/${conn.id.slice(0, 8)} quota OK → cleared warning`);
          }
          continue;
        }
        ...
        console.log(`[QuotaMonitor] ${provider}/${conn.id.slice(0, 8)} WARNING: ${warningMessage} (staying active, waiting for chat confirmation)`);

      } catch (e) {
        console.warn(`[QuotaMonitor] error processing ${provider}/${conn.id.slice(0, 8)}: ${e.message}`);
      }
    }
  }
    ...
    for (const conn of expired) {
      try {
        ...
        if (usageData && isQuotaRecovered(provider, usageData)) {
          ...
          await clearQuotaWarning(conn.id);
          QuotaStore.set(conn.id, usageData, 10 * 60 * 1000);
          console.log(`[QuotaMonitor] ${provider}/${conn.id.slice(0, 8)} RECOVERED → activated`);
        } else {
          await lifecyclePause(conn.id, MAX_PAUSE_WITHOUT_RESET_MS);
          console.log(`[QuotaMonitor] ${provider}/${conn.id.slice(0, 8)} still exhausted → re-paused 24h`);
        }
      } catch (e) {
        console.warn(`[QuotaMonitor] recovery check failed ${provider}/${conn.id.slice(0, 8)}: ${e.message}`);
      }
    }
  }
}
